...
//		Reweighing_ResetContTrainingSameWeight(infile,outfile);
		...
		//3. (NYY) No Reset + Continue Training + Apply New Weight after Reweighing: long time
...
//						learner.resetLearning();
	...
	public static void ApplyReweighing(int[] posWindow, String[] labels, int[] trueLabels){
		int sav=tpDeprived+fpDeprived+tnDeprived+fnDeprived;
		int fav=tpFavored+fpFavored+tnFavored+fnFavored;
		int pos=tpDeprived+fpDeprived+tpFavored+fpFavored;
		int neg=tnDeprived+fnDeprived+tnFavored+fnFavored;
		//weight calculation
		if ((tpDeprived+fpDeprived)!=0)
			savPos=(double)sav*(double)pos/(double)(windowSize*(tpDeprived+fpDeprived));
		else
			savPos=1;
		if ((tnDeprived+fnDeprived)!=0)
			savNeg=(double)sav*(double)neg/(double)(windowSize*(tnDeprived+fnDeprived));
		else
			savNeg=1;
		if ((tpFavored+fpFavored)!=0)
			favPos=(double)fav*(double)pos/(double)(windowSize*(tpFavored+fpFavored));
		else
			favPos=1;
		if ((tnFavored+fnFavored)!=0)
			favNeg=(double)fav*(double)neg/(double)(windowSize*(tnFavored+fnFavored));
		else
			favNeg=1;
